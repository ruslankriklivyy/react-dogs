...
const VotePage = observer(() => {
  const { voteStore, galleryStore } = useRootStore();

  const setVote = (page: number) => {
    galleryStore.addImageToFavorites(voteStore.currentVoteImage[0].id);
    voteStore.setCurrentPage(page);
  };

  React.useEffect(() => {
    voteStore.getCurrentVoteImage(voteStore.currentPage);
  }, [voteStore, voteStore.currentPage]);
  ...
  return (
    <div className="vote">
      ...
      <div className="box-white">
        ...
        {voteStore.isFetching ? (
          <>
            <div className="vote-dog">
              <div className="vote-dog__img">
                <img src={voteStore.currentVoteImage[0].url} alt="dog jpg" />
                <div className="vote-actions">
                  {iconsArr.map((item) => (
                    <button
                      key={item.type}
                      onClick={() => setVote(voteStore.currentPage + 1)}
